...
def generate_mrf_case(
    ...
    data = data.reshape(smaps.shape[0], *k.shape[:-1])
...
def generate_spgr_case(
    ...
    grog_training = sigpy.resize(grog_training, oshape=grog_training.shape[:-2] + [24, 24])
    ...
    data = nufft.op(segmentation)
    data = data.sum(axis=0)
    data = data.reshape(*smaps.shape[:-2], *k.shape[:-1])
